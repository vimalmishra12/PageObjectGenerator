...
function generateTestcase(pageSelectorFile, inputFile, pageSelectorGroup, appShellPageCheck) {
    ...
    for (var i = 0; i < pageSelectorFile.length; i++) {
        if ((pageSelectorFile[i].tagName).toLowerCase().includes("button")) {
            console.log("TST_" + (inputFile.substring(0, 4)).toUpperCase() + "_TC_" + testCaseNumber + " :   async function (testdata) { \n");
            file2.write("TST_" + (inputFile.substring(0, 4)).toUpperCase() + "_TC_" + testCaseNumber + " :   async function (testdata) { \n")
